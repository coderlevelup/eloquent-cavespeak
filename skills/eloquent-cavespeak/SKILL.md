---
name: eloquent-cavespeak
description: >
  Start the eloquent-cavespeak two-layer pipeline: Claude compresses output via cavespeak,
  then a local Gemma proxy rehydrates it into polished prose before it reaches the user.
  Cuts API token costs while preserving output quality. Use when user says
  "/eloquent-cavespeak", "start rehydration", or "start the proxy".
---
